...
public class Caldero {
    ...
    public Caldero (String nombre, Integer capacidad) {
        ...
        pocima = new Elemento(null, null);
        MAX_INGREDIENTES = capacidad; // Ojo, esta linea no es valida
    }
    ...
    public void setReceta (Receta receta) {
        if ((receta.getCantidadIngredientes() <= MAX_INGREDIENTES) && (receta.equals(null)) && (pocima.equals(null)))
        {
            ...
            System.out.println(nombre + ": No se puede agregar " + receta.getNombre());
        }
    ...
    public List<String> getIngredientesFaltantes () {
        ...
        for(int i = 0; i<9; i++){
            if(!(elementos.containsKey(i) == elementos.containsValue(i))){
                List<String> lista = elemento(i; 
                return lista;
            }
        }
        return null;
